...
int main(int argc, char *argv[])
{
    ...
    if (variableType == "float")
    {
        ...
        adios2::Engine rawdata_reader_engine =
            reader_io.Open(rawDataFileName, adios2::Mode::ReadRandomAccess); 
        //rawdata_reader_engine.BeginStep();  
                ...
                for (size_t i = 0; i < dataTiersValues.size(); i++)
                {
                    ...
                    for (size_t j = 0; j < dataTiersECParam_k[i]; j++)
                    {
                        ...
                        adios2::Engine data_reader_engine =
                            reader_io.Open(dataTiersDataLocations[i][j], adios2::Mode::ReadRandomAccess); 
                        //adios2::Engine data_reader_engine =
                        //    reader_io.Open(dataTiersDataLocations[i][j], adios2::Mode::ReadRandomAccess);
                        ...
                        std::cout << "shape" << varDataValues.Shape()[0] << std::endl;
                        std::cout << "avail_frags: "<< avail_frags[num_avail_frags] << std::endl;
                        std::cout << varDataValues << std::endl;
                    ...
                    for (size_t j = 0; j < dataTiersECParam_m[i]; j++)
                    {
                        ...
                        adios2::Engine parity_reader_engine =
                            reader_io.Open(dataTiersParityLocations[i][j], adios2::Mode::ReadRandomAccess); 
