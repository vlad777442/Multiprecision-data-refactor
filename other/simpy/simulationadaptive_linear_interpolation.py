...
class Sender:
    ...
    def calculate_packet_loss(self, received_fragments_count, transmission_time, fragments_sent):
        ...
        if lost_fragments > 0:
            ...
            self.m_history.append(self.tier_m.copy())
            predicted_m = self.predict_m_values(new_lambda)
    ...
    def predict_m_values(self, new_lambda):
        """Predict m values using linear interpolation based on historical data."""
        if len(self.lambda_history) < 2:
            # Not enough historical data, use the calculator to find the best m
            _, best_m, _ = self.calculator.find_min_time_configuration()
            return best_m

        # Find the two closest lambda values
        # closest_indices = sorted(range(len(self.lambda_history)), 
        #                          key=lambda i: abs(self.lambda_history[i] - new_lambda))[:2]
        
        # lambda1, lambda2 = [self.lambda_history[i] for i in closest_indices]
        lambda1, lambda2 = self.lambda_history[-2], self.lambda_history[-1]
        print(f"Closest lambdas: {lambda1}, {lambda2}")
        # m1, m2 = [self.m_history[i] for i in closest_indices]
        m1, m2 = self.m_history[-2], self.m_history[-1]

        # Perform linear interpolation for each tier
        predicted_m = {}
        for tier in self.tier_m.keys():
            y1, y2 = m1[tier], m2[tier]
            
            if lambda1 == lambda2:
                # If lambdas are equal, use the most recent m value
                predicted_m[tier] = y2
            else:
                # Perform linear interpolation
                predicted_m[tier] = int(y1 + (new_lambda - lambda1) * (y2 - y1) / (lambda2 - lambda1))
            
            # Ensure the predicted m is within valid bounds
            predicted_m[tier] = max(0, min(predicted_m[tier], self.n - 1))
